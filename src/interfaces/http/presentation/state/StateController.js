...
module.exports = container => ({
    ...
    getAll: AsyncMiddleware(async request => {
        const response = await container.getAllStateOperation.execute(request.query);
        return request.res.status(container.httpConstants.code.OK).json(response);
    }),
    ...
    update: AsyncMiddleware(async request => {
        const { id } = request.params;
        const { body } = request;
        const response = await container.updateStateOperation.execute(id, body);
        return request.res.status(container.httpConstants.code.OK).json(response);
    }),

    delete: AsyncMiddleware(async request => {
        const { id } = request.params;
        const response = await container.deleteStateOperation.execute(id);
        return request.res.status(container.httpConstants.code.OK).json(response);
    })
});
